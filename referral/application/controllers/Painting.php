...
class Painting extends BaseController {
	...
	public function __construct()
	{
		...
		$this->isLoggedIn();  
		$this->load->model('Painting_model','painting');
	}

	public function Painting()
	{
		$this->load->helper('url');
       $this->global['pageTitle'] = 'CodeInsect : Painting';
		$this->loadViews('Painting', $this->global, NULL , NULL);
	...
	public function ajax_list()
	{
		...
		$list = $this->painting->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $painting) {
			$no++;
			$row = array();
			if($painting->pic)
				$row[] = '<a href="'.base_url('upload/'.$painting->pic).'" target="_blank"><img src="'.base_url('upload/'.$painting->pic).'" style="width:75PX;height:75px;border-radius:50%;" class="img-responsive" /></a>';
			else
				$row[] = '(No photo)';
			$row[] = $painting->sl;
			$row[] = $painting->name;
			$row[] = $painting->email;
			$row[] = $painting->phone;
			$row[] = $painting->location;
			$row[] = $painting->designation;
			$row[] = $painting->emp_id;
			$row[] = $painting->join_date;
			$row[] = $painting->level;
			$row[] = $painting->jobs_done;
			$row[] = $painting->age;
			
			$row[] = $painting->expected_amount;
			$row[] = $painting->service_status;
			$row[] = $painting->sub_cat;
			

			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$painting->electrician_id."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$painting->electrician_id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->painting->count_all(),
						"recordsFiltered" => $this->painting->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->painting->get_by_id($electrician_id);
	...
	{
		$data = $this->painting->get_by_id1($electrician_id);
	...
	public function ajax_update()
	{
		...
		if(!empty($_FILES['pic']['name']))
		{
			...
			//delete file
			$painting = $this->painting->get_by_id($this->input->post('electrician_id'));
			if(file_exists('upload/'.$painting->pic) && $painting->pic)
				unlink('upload/'.$painting->pic);

			$data['pic'] = $upload;
		}

		$this->painting->update(array('electrician_id' => $this->input->post('electrician_id')), $data);
		echo json_encode(array("status" => TRUE));
	}
	...
	{
		//delete file
		$painting = $this->painting->get_by_id($electrician_id);
		if(file_exists('upload/'.$painting->pic) && $painting->pic)
			unlink('upload/'.$painting->pic);
		
		$this->painting->delete_by_id($electrician_id);
		echo json_encode(array("status" => TRUE));
	}
