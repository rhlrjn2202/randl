...
class Acrepairs extends BaseController {
	...
	public function __construct()
	{
		...
		$this->isLoggedIn();  
		$this->load->model('Acrepairs_model','acrepairs');
	}

	public function ACrepairs()
	{
		$this->load->helper('url');
       $this->global['pageTitle'] = 'CodeInsect : ACrepairs';
		$this->loadViews('ACrepair', $this->global, NULL , NULL);
	...
	public function ajax_list()
	{
		...
		$list = $this->acrepairs->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $acrepairs) {
			$no++;
			$row = array();
			if($acrepairs->pic)
				$row[] = '<a href="'.base_url('upload/'.$acrepairs->pic).'" target="_blank"><img src="'.base_url('upload/'.$acrepairs->pic).'" style="width:75PX;height:75px;border-radius:50%;" class="img-responsive" /></a>';
			else
				$row[] = '(No photo)';
			$row[] = $acrepairs->sl;
			$row[] = $acrepairs->name;
			$row[] = $acrepairs->email;
			$row[] = $acrepairs->phone;
			$row[] = $acrepairs->location;
			$row[] = $acrepairs->designation;
			$row[] = $acrepairs->emp_id;
			$row[] = $acrepairs->join_date;
			$row[] = $acrepairs->level;
			$row[] = $acrepairs->jobs_done;
			$row[] = $acrepairs->age;
			
			$row[] = $acrepairs->expected_amount;
			$row[] = $acrepairs->service_status;
			$row[] = $acrepairs->sub_cat;
			

			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$acrepairs->electrician_id."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$acrepairs->electrician_id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->acrepairs->count_all(),
						"recordsFiltered" => $this->acrepairs->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->acrepairs->get_by_id($electrician_id);
	...
	{
		$data = $this->acrepairs->get_by_id1($electrician_id);
	...
	public function ajax_add()
	{
		...
		$insert = $this->acrepairs->save($data);
	...
	public function ajax_update()
	{
		...
		if(!empty($_FILES['pic']['name']))
		{
			...
			//delete file
			$acrepairs = $this->acrepairs->get_by_id($this->input->post('electrician_id'));
			if(file_exists('upload/'.$acrepairs->pic) && $acrepairs->pic)
				unlink('upload/'.$acrepairs->pic);

			$data['pic'] = $upload;
		}

		$this->acrepairs->update(array('electrician_id' => $this->input->post('electrician_id')), $data);
		echo json_encode(array("status" => TRUE));
	}
	...
	{
		//delete file
		$acrepairs = $this->acrepairs->get_by_id($electrician_id);
		if(file_exists('upload/'.$acrepairs->pic) && $acrepairs->pic)
			unlink('upload/'.$acrepairs->pic);
		
		$this->acrepairs->delete_by_id($electrician_id);
		echo json_encode(array("status" => TRUE));
	}
