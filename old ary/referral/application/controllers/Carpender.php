...
class Carpender extends BaseController {
	...
	public function __construct()
	{
		...
		$this->isLoggedIn();  
		$this->load->model('Carpender_model','carpender');
	}

	public function Carpenter()
	{
		$this->load->helper('url');
       $this->global['pageTitle'] = 'CodeInsect : Carpender';
		$this->loadViews('carpenter', $this->global, NULL , NULL);
	...
	public function ajax_list()
	{
		...
		$list = $this->carpender->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $carpender) {
			$no++;
			$row = array();
			if($carpender->pic)
				$row[] = '<a href="'.base_url('upload/'.$carpender->pic).'" target="_blank"><img src="'.base_url('upload/'.$carpender->pic).'" style="width:75PX;height:75px;border-radius:50%;" class="img-responsive" /></a>';
			else
				$row[] = '(No photo)';
			$row[] = $carpender->sl;
			$row[] = $carpender->name;
			$row[] = $carpender->email;
			$row[] = $carpender->phone;
			$row[] = $carpender->location;
			$row[] = $carpender->designation;
			$row[] = $carpender->emp_id;
			$row[] = $carpender->join_date;
			$row[] = $carpender->level;
			$row[] = $carpender->jobs_done;
			$row[] = $carpender->age;
			
			$row[] = $carpender->expected_amount;			
			$row[] = $carpender->service_status;
			$row[] = $carpender->sub_cat;
			

			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$carpender->electrician_id."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$carpender->electrician_id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->carpender->count_all(),
						"recordsFiltered" => $this->carpender->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->carpender->get_by_id($electrician_id);
	...
	{
		$data = $this->carpender->get_by_id1($electrician_id);
	...
	public function ajax_add()
	{
		...
		$insert = $this->carpender->save($data);
	...
	public function ajax_update()
	{
		...
		if(!empty($_FILES['pic']['name']))
		{
			...
			//delete file
			$carpender = $this->carpender->get_by_id($this->input->post('electrician_id'));
			if(file_exists('upload/'.$carpender->pic) && $carpender->pic)
				unlink('upload/'.$carpender->pic);

			$data['pic'] = $upload;
		}

		$this->carpender->update(array('electrician_id' => $this->input->post('electrician_id')), $data);
		echo json_encode(array("status" => TRUE));
	}
	...
	{
		//delete file
		$carpender = $this->carpender->get_by_id($electrician_id);
		if(file_exists('upload/'.$carpender->pic) && $carpender->pic)
			unlink('upload/'.$carpender->pic);
		
		$this->carpender->delete_by_id($electrician_id);
		echo json_encode(array("status" => TRUE));
	}
