...
class Cleaning extends BaseController {
	...
	public function Cleaning()
	{
       ...
       $this->global['pageTitle'] = 'CodeInsect : Cleaning';
		$this->loadViews('Cleaning', $this->global, NULL , NULL);
	...
	public function ajax_list()
	{
		...
		$list = $this->cleaning->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $cleaning) {
			$no++;
			$row = array();
			if($cleaning->pic)
				$row[] = '<a href="'.base_url('upload/'.$cleaning->pic).'" target="_blank"><img src="'.base_url('upload/'.$cleaning->pic).'" style="width:75PX;height:75px;border-radius:50%;" class="img-responsive" /></a>';
			else
				$row[] = '(No photo)';
			$row[] = $cleaning->sl;
			$row[] = $cleaning->name;
			$row[] = $cleaning->email;
			$row[] = $cleaning->phone;
			$row[] = $cleaning->location;
			$row[] = $cleaning->designation;
			$row[] = $cleaning->emp_id;
			$row[] = $cleaning->join_date;
			$row[] = $cleaning->level;
			$row[] = $cleaning->jobs_done;
			$row[] = $cleaning->age;
			
			$row[] = $cleaning->expected_amount;
			$row[] = $cleaning->service_status;
			$row[] = $cleaning->sub_cat;
			

			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$cleaning->electrician_id."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$cleaning->electrician_id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->cleaning->count_all(),
						"recordsFiltered" => $this->cleaning->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->cleaning->get_by_id($electrician_id);
	...
	{
		$data = $this->cleaning->get_by_id1($electrician_id);
	...
	public function ajax_add()
	{
		...
		$insert = $this->cleaning->save($data);
	...
	public function ajax_update()
	{
		...
		if(!empty($_FILES['pic']['name']))
		{
			...
			//delete file
			$cleaning = $this->cleaning->get_by_id($this->input->post('electrician_id'));
			if(file_exists('upload/'.$cleaning->pic) && $cleaning->pic)
				unlink('upload/'.$cleaning->pic);

			$data['pic'] = $upload;
		}

		$this->cleaning->update(array('electrician_id' => $this->input->post('electrician_id')), $data);
		echo json_encode(array("status" => TRUE));
	}
	...
	{
		//delete file
		$cleaning = $this->cleaning->get_by_id($electrician_id);
		if(file_exists('upload/'.$cleaning->pic) && $cleaning->pic)
			unlink('upload/'.$cleaning->pic);
		
		$this->cleaning->delete_by_id($electrician_id);
		echo json_encode(array("status" => TRUE));
	}
