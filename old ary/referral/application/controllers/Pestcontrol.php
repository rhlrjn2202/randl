...
class Pestcontrol extends BaseController {
	...
	public function __construct()
	{
		...
		$this->isLoggedIn();  
		$this->load->model('Pestcontrol_model','pestcontrol');
	}

	public function Pestcontrol()
	{
		$this->load->helper('url');
       $this->global['pageTitle'] = 'CodeInsect : Pestcontrol';
		$this->loadViews('Pestcontrol', $this->global, NULL , NULL);
	...
	public function ajax_list()
	{
		...
		$list = $this->pestcontrol->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $pestcontrol) {
			$no++;
			$row = array();
			if($pestcontrol->pic)
				$row[] = '<a href="'.base_url('upload/'.$pestcontrol->pic).'" target="_blank"><img src="'.base_url('upload/'.$pestcontrol->pic).'" style="width:75PX;height:75px;border-radius:50%;" class="img-responsive" /></a>';
			else
				$row[] = '(No photo)';
			$row[] = $pestcontrol->sl;
			$row[] = $pestcontrol->name;
			$row[] = $pestcontrol->email;
			$row[] = $pestcontrol->phone;
			$row[] = $pestcontrol->location;
			$row[] = $pestcontrol->designation;
			$row[] = $pestcontrol->emp_id;
			$row[] = $pestcontrol->join_date;
			$row[] = $pestcontrol->level;
			$row[] = $pestcontrol->jobs_done;
			$row[] = $pestcontrol->age;
			
			$row[] = $pestcontrol->expected_amount;
			$row[] = $pestcontrol->service_status;
			$row[] = $pestcontrol->sub_cat;
			

			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$pestcontrol->electrician_id."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$pestcontrol->electrician_id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->pestcontrol->count_all(),
						"recordsFiltered" => $this->pestcontrol->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->pestcontrol->get_by_id($electrician_id);
	...
	{
		$data = $this->pestcontrol->get_by_id1($electrician_id);
	...
	public function ajax_add()
	{
		...
		$insert = $this->pestcontrol->save($data);
	...
	public function ajax_update()
	{
		...
		if(!empty($_FILES['pic']['name']))
		{
			...
			//delete file
			$pestcontrol = $this->pestcontrol->get_by_id($this->input->post('electrician_id'));
			if(file_exists('upload/'.$pestcontrol->pic) && $pestcontrol->pic)
				unlink('upload/'.$pestcontrol->pic);

			$data['pic'] = $upload;
		}

		$this->pestcontrol->update(array('electrician_id' => $this->input->post('electrician_id')), $data);
		echo json_encode(array("status" => TRUE));
	}
	...
	{
		//delete file
		$pestcontrol = $this->pestcontrol->get_by_id($electrician_id);
		if(file_exists('upload/'.$pestcontrol->pic) && $pestcontrol->pic)
			unlink('upload/'.$pestcontrol->pic);
		
		$this->pestcontrol->delete_by_id($electrician_id);
		echo json_encode(array("status" => TRUE));
	}
