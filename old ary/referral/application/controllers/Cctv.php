...
class Cctv extends BaseController {
	...
	public function CCTV()
	{
       ...
       $this->global['pageTitle'] = 'CodeInsect : CCTV';
		$this->loadViews('CCTV', $this->global, NULL , NULL);
	...
	public function ajax_list()
	{
		...
		$list = $this->cctv->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $cctv) {
			$no++;
			$row = array();
			if($cctv->pic)
				$row[] = '<a href="'.base_url('upload/'.$cctv->pic).'" target="_blank"><img src="'.base_url('upload/'.$cctv->pic).'" style="width:75PX;height:75px;border-radius:50%;" class="img-responsive" /></a>';
			else
				$row[] = '(No photo)';
			$row[] = $cctv->sl;
			$row[] = $cctv->name;
			...
			$row[] = $cctv->expected_amount;
			$row[] = $cctv->service_status;
			$row[] = $cctv->sub_cat;
			

			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$cctv->electrician_id."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$cctv->electrician_id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
	...
	{
		$data = $this->cctv->get_by_id($electrician_id);
	...
	{
		$data = $this->cctv->get_by_id1($electrician_id);
	...
	public function ajax_update()
	{
		...
		if(!empty($_FILES['pic']['name']))
		{
			...
			//delete file
			$cctv = $this->cctv->get_by_id($this->input->post('electrician_id'));
			if(file_exists('upload/'.$cctv->pic) && $cctv->pic)
				unlink('upload/'.$cctv->pic);

			$data['pic'] = $upload;
		}

		$this->cctv->update(array('electrician_id' => $this->input->post('electrician_id')), $data);
		echo json_encode(array("status" => TRUE));
	}
	...
	{
		//delete file
		$cctv = $this->cctv->get_by_id($electrician_id);
		if(file_exists('upload/'.$cctv->pic) && $cctv->pic)
			unlink('upload/'.$cctv->pic);
		
		$this->cctv->delete_by_id($electrician_id);
		echo json_encode(array("status" => TRUE));
	}
